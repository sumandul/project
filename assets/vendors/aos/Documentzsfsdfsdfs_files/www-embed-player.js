...
r(Ig,J);Ig.prototype.getKey=function(){return Zc(this,1)};
...
r(Ng,J);Ng.prototype.getPlayerType=function(){return Zc(this,36)};
...
r(Yg,J);Yg.prototype.getKey=function(){return jd(this,1)};
...
r(bh,J);bh.prototype.getMessage=function(){return jd(this,1)};function ch(a){J.call(this,a)}
...
r(wh,J);wh.prototype.getPlayerType=function(){var a=Zc(this,7);return null==a?0:a};
wh.prototype.setVideoId=function(a){return D(this,19,a)};
function yh(a,b){id(a,68,zh,b)}
function zh(a){J.call(this,a)}
r(zh,J);zh.prototype.getId=function(){return jd(this,2)};
...
r(Gh,J);Gh.prototype.setVideoId=function(a){return ad(this,1,Hh,a)};
...
function Hj(a,b,c){a=a.bb;if("WEB"===a||"MWEB"===a||1===a||2===a)if(b){c=fd(b,Lg,96)||new Lg;var d=nj();null!==d&&D(c,3,d);I(b,96,c)}else c&&(c.client.mainAppWebInfo=null!=(d=c.client.mainAppWebInfo)?d:{},c.client.mainAppWebInfo.webDisplayMode=mj())}
function Ij(a,b,c){a=a.bb;if(("WEB_REMIX"===a||76===a)&&!M("music_web_display_mode_killswitch"))if(b){var d;c=null!=(d=fd(b,Mg,70))?d:new Mg;d=nj();null!==d&&D(c,10,d);I(b,70,c)}else if(c){var e;c.client.Ab=null!=(e=c.client.Ab)?e:{};c.client.Ab.webDisplayMode=mj()}}
...
function Kj(a,b,c){if(a.appInstallData)if(b){var d;c=null!=(d=fd(b,Kg,62))?d:new Kg;D(c,6,a.appInstallData);I(b,62,c)}else c&&(c.client.configInfo=c.client.configInfo||{},c.client.configInfo.appInstallData=a.appInstallData)}
...
function Wo(a,b){var c="";if(a.va)c="visitorOnlyApprovedKey";else if(a.cttAuthInfo){if(void 0===b?0:b){b=a.cttAuthInfo.token;c=a.cttAuthInfo;var d=new Gh;c.videoId?d.setVideoId(c.videoId):c.playlistId&&ad(d,2,Hh,c.playlistId);Uo[b]=d}else b=a.cttAuthInfo,c={},b.videoId?c.videoId=b.videoId:b.playlistId&&(c.playlistId=b.playlistId),To[a.cttAuthInfo.token]=c;c=a.cttAuthInfo.token}return c}
...
a?Po.h=b:Oo.h=b}Di(a?Po.i:Oo.i);b=A("LOGGING_BATCH_TIMEOUT",ci("web_gel_debounce_ms",1E4));M("shorten_initial_gel_batch_timeout")&&Qo&&(b=mo);b=Ci(function(){Zo({writeThenSend:!0},void 0,a)},b);
a?Po.i=b:Oo.i=b}
...
g||b()},d));
Qo=!1}}
function dp(a,b,c,d){var e=Mo;c=void 0===c?{}:c;var f=Math.round(P()),g=a.size;a=p(a);for(var h=a.next();!h.done;h=a.next()){var k=p(h.value);h=k.next().value;var m=k=k.next().value;k=new Ih;var q=Mj(e.config_||Fj());I(k,1,q);m=ip(m);for(q=0;q<m.length;q++)id(k,3,Dh,m[q]);(m=Uo[h])&&jp(k,h,m);delete Uo[h];h="visitorOnlyApprovedKey"===h;kp(k,f,h);gp(c);k=td(k);h=hp(c,h,function(){g--;g||b()},function(){g--;
g||b()},d);
...
function jp(a,b,c){if(Zc(c,1===bd(c,Hh)?1:-1))var d=1;else if(c.getPlaylistId())d=2;else return;I(a,4,c);a=fd(a,Ug,1)||new Ug;c=fd(a,Sg,3)||new Sg;var e=new Rg;e.setToken(b);D(e,1,d);id(c,12,Rg,e);I(a,3,c)}
...
":"+Ba);if("IGNORED"===a.level)var sa=0;else a:{for(var Wn=up(),Xn=p(Wn.ha),Gi=Xn.next();!Gi.done;Gi=Xn.next()){var Yn=Gi.value;if(G.message&&G.message.match(Yn.Jo)){sa=Yn.weight;break a}}for(var Zn=p(Wn.ga),Hi=Zn.next();!Hi.done;Hi=Zn.next()){var $n=Hi.value;if($n.ka(G)){sa=$n.weight;break a}}sa=1}G.sampleWeight=sa;for(var ao=p(pp),Ii=ao.next();!Ii.done;Ii=ao.next()){var Ji=Ii.value;if(Ji.Pa[G.name])for(var bo=p(Ji.Pa[G.name]),Ki=bo.next();!Ki.done;Ki=bo.next()){var co=Ki.value,Sf=G.message.match(co.regexp);
if(Sf){G.params["params.error.original"]=Sf[0];for(var Li=co.groups,eo={},cd=0;cd<Li.length;cd++)eo[Li[cd]]=Sf[cd+1],G.params["params.error."+Li[cd]]=Sf[cd+1];G.message=Ji.fb(eo);break}}}G.params||(G.params={});var fo=up();G.params["params.errorServiceSignature"]="msg="+fo.ha.length+"&cb="+fo.ga.length;G.params["params.serviceWorker"]="false";x.document&&x.document.querySelectorAll&&(G.params["params.fscripts"]=String(document.querySelectorAll("script:not([nonce])").length));Db("sample").constructor!==
Bb&&(G.params["params.fconst"]="true");window.yterr&&"function"===typeof window.yterr&&window.yterr(G);if(0!==G.sampleWeight&&!Lp.has(G.message)){if("ERROR"===h){vp.ja("handleError",G);if(M("record_app_crashed_web")&&0===Op&&1===G.sampleWeight)if(Op++,M("errors_via_jspb")){var Mi=new gh;D(Mi,1,1);if(!M("report_client_error_with_app_crash_ks")){var go=new bh;D(go,1,G.message);var ho=new ch;I(ho,3,go);var io=new dh;I(io,5,ho);var jo=new eh;I(jo,9,io);I(Mi,4,jo)}var ko=new Dh;hd(ko,20,Eh,Mi);Dp(ko,void 0)}else{var lo=
{appCrashType:"APP_CRASH_TYPE_BREAKPAD"};M("report_client_error_with_app_crash_ks")||(lo.systemHealth={crashData:{clientError:{logMessage:{message:G.message}}}});ok("appCrashed",lo)}Np++}else"WARNING"===h&&vp.ja("handleWarning",G);if(M("kevlar_gel_error_routing"))a:{var ae=h;if(M("errors_via_jspb")){if(Tp())var no=void 0;else{var dd=new Zg;D(dd,1,G.stack);G.fileName&&D(dd,4,G.fileName);var sb=G.lineNumber&&G.lineNumber.split?G.lineNumber.split(":"):[];0!==sb.length&&(1!==sb.length||isNaN(Number(sb[0]))?
2!==sb.length||isNaN(Number(sb[0]))||isNaN(Number(sb[1]))||(D(dd,2,Number(sb[0])),D(dd,3,Number(sb[1]))):D(dd,2,Number(sb[0])));var nc=new bh;D(nc,1,G.message);D(nc,3,G.name);D(nc,6,G.sampleWeight);"ERROR"===ae?D(nc,2,2):"WARNING"===ae?D(nc,2,1):D(nc,2,0);var Ni=new $g;D(Ni,1,!0);hd(Ni,3,ah,dd);var Nb=new Wg;D(Nb,3,window.location.href);for(var oo=A("FEXP_EXPERIMENTS",[]),Oi=0;Oi<oo.length;Oi++){var Ft=oo[Oi];Wc(Nb);$c(Nb,5).push(Ft)}var Pi=A("LATEST_ECATCHER_SERVICE_TRACKING_PARAMS",void 0);if(!ai()&&
Pi)for(var po=p(Object.keys(Pi)),oc=po.next();!oc.done;oc=po.next()){var qo=oc.value,Qi=new Yg;D(Qi,1,qo);Qi.setValue(String(Pi[qo]));id(Nb,4,Yg,Qi)}var Ri=G.params;if(Ri){var ro=p(Object.keys(Ri));for(oc=ro.next();!oc.done;oc=ro.next()){var so=oc.value,Si=new Yg;D(Si,1,"client."+so);Si.setValue(String(Ri[so]));id(Nb,4,Yg,Si)}}var to=$h("SERVER_NAME"),uo=$h("SERVER_VERSION");if(to&&uo){var Ti=new Yg;D(Ti,1,"server.name");Ti.setValue(to);id(Nb,4,Yg,Ti);var Ui=new Yg;D(Ui,1,"server.version");Ui.setValue(uo);
id(Nb,4,Yg,Ui)}var Tf=new ch;I(Tf,1,Nb);I(Tf,2,Ni);I(Tf,3,nc);no=Tf}var vo=no;if(!vo)break a;var wo=new Dh;hd(wo,163,Eh,vo);Dp(wo,void 0)}else{if(Tp())var xo=void 0;else{var be={stackTrace:G.stack};G.fileName&&(be.filename=G.fileName);var tb=G.lineNumber&&G.lineNumber.split?G.lineNumber.split(":"):[];0!==tb.length&&(1!==tb.length||isNaN(Number(tb[0]))?2!==tb.length||isNaN(Number(tb[0]))||isNaN(Number(tb[1]))||(be.lineNumber=Number(tb[0]),be.columnNumber=Number(tb[1])):be.lineNumber=Number(tb[0]));
var Vi={level:"ERROR_LEVEL_UNKNOWN",message:G.message,errorClassName:G.name,sampleWeight:G.sampleWeight};"ERROR"===ae?Vi.level="ERROR_LEVEL_ERROR":"WARNING"===ae&&(Vi.level="ERROR_LEVEL_WARNNING");var Gt={isObfuscated:!0,browserStackInfo:be},ed={pageUrl:window.location.href,kvPairs:[]};A("FEXP_EXPERIMENTS")&&(ed.experimentIds=A("FEXP_EXPERIMENTS"));var Wi=A("LATEST_ECATCHER_SERVICE_TRACKING_PARAMS",void 0);if(!ai()&&Wi)for(var yo=p(Object.keys(Wi)),pc=yo.next();!pc.done;pc=yo.next()){var zo=pc.value;
ed.kvPairs.push({key:zo,value:String(Wi[zo])})}var Xi=G.params;if(Xi){var Ao=p(Object.keys(Xi));for(pc=Ao.next();!pc.done;pc=Ao.next()){var Bo=pc.value;ed.kvPairs.push({key:"client."+Bo,value:String(Xi[Bo])})}}var Co=$h("SERVER_NAME"),Do=$h("SERVER_VERSION");Co&&Do&&(ed.kvPairs.push({key:"server.name",value:Co}),ed.kvPairs.push({key:"server.version",value:Do}));xo={errorMetadata:ed,stackTrace:Gt,logMessage:Vi}}var Eo=xo;if(!Eo)break a;ok("clientError",Eo)}("ERROR"===ae||M("errors_flush_gel_always_killswitch"))&&
Zo()}if(!M("suppress_error_204_logging")){var ce=G.params||{},Ob={urlParams:{a:"logerror",t:"jserror",type:G.name,msg:G.message.substr(0,250),line:G.lineNumber,level:h,"client.name":ce.name},postParams:{url:A("PAGE_NAME",window.location.href),file:G.fileName},method:"POST"};ce.version&&(Ob["client.version"]=ce.version);if(Ob.postParams){G.stack&&(Ob.postParams.stack=G.stack);for(var Fo=p(Object.keys(ce)),Yi=Fo.next();!Yi.done;Yi=Fo.next()){var Go=Yi.value;Ob.postParams["client."+Go]=ce[Go]}var Zi=
A("LATEST_ECATCHER_SERVICE_TRACKING_PARAMS",void 0);if(Zi)for(var Ho=p(Object.keys(Zi)),$i=Ho.next();!$i.done;$i=Ho.next()){var Io=$i.value;Ob.postParams[Io]=Zi[Io]}var Jo=A("SERVER_NAME",void 0),Ko=A("SERVER_VERSION",void 0);Jo&&Ko&&(Ob.postParams["server.name"]=Jo,Ob.postParams["server.version"]=Ko)}dj(A("ECATCHER_REPORT_HOST","")+"/error_204",Ob)}try{Lp.add(G.message)}catch(ev){}Mp++}}}}
...
$r.prototype.tick=function(a,b,c,d){bs(this,"tick_"+a+"_"+b)||(c={timestamp:c,cttAuthInfo:d},M("web_csi_via_jspb")?(d=new Ah,D(d,1,a),D(d,2,b),a=new Dh,hd(a,5,Eh,d),Dp(a,c)):ok("latencyActionTicked",{tickName:a,clientActionNonce:b},c))};
...
W:b};d=p(d);for(var g=d.next();!g.done;g=d.next())g=g.value.getAsJson(),(pb(g)||!g.trackingParams&&!g.veType)&&Sp(Error("Child VE logged with no data"));if(M("il_via_jspb")){var h=new oh;h.X(b);qh(h,c.getAsJspb());hb(e,function(k){k=k.getAsJspb();id(h,3,hh,k)});
